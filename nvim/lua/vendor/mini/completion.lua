...
MiniCompletion.config = {
    ...
    -- Used to, optionally, completely disable the automatic
...
H.setup_config = function(config)
    ...
    if not is_string_or_array(config.window.signature.border or 'single') then
        H.error(
            '`config.window.signature.border` should be either string or array, not ' ..
            type(config.window.signature.border)
        )
    end
...
H.create_autocommands = function(config)
    ...
    if not config.disable_auto then
...
H.normalize_item_doc = function(lsp_item, fallback_info)
    ...
    detail = (H.is_whitespace(detail) or doc:find(detail, 1, true) ~= nil) and '' or
    (H.wrap_in_codeblock(detail) .. '\n')
